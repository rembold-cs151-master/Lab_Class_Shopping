...
class Test_Part2:
    ...
    def test_print_attributes_created(self):
        student = Lab21.Item(*self.entries)
        assert 'print_attributes' in dir(student), 'Did you add a method called print_attributes?'
    ...
    def test_printed_includes_all(self, capsys):
        ...
        for i,entry in enumerate(self.entries):
            assert str(entry) in captured, f'It looks like the {names[i]} of the Item is not being printed to the screen?'
...
class Test_Part3:
    ...
    def test_purchase_works_properly(self, capsys):
        ...
        assert 17.7 < amount < 17.9, 'You are not returning the proper amount to change them?'
        amount = student.purchase(15)
        assert student.stock == 0, 'Your stock should be 0 but is not?'
        assert 21.35 < amount < 21.37, 'You should only be charging them for what is left in stock!'
